...
class OverwaiteaSpider(scrapy.Spider):
		name = "overwaitea"
		uid_list = []
		start_urls = ['https://www.overwaitea.com/store-finder-map']

		def parse(self, response):
			pdb.set_trace()
			start = response.body.find('jQuery.extend(Drupal.settings, ') + 31
			end = response.body.find('})', start) + 1
			stores = yaml.load(response.body[start:end])["gmap"]["auto1map"]["markers"]
