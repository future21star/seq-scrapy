...
class PartycitySpider(scrapy.Spider):
    name = 'partycity'
    domain = 'http://stores.partycity.ca/'
    history = ['']

    def start_requests(self):
        url = 'http://stores.partycity.ca/'
    ...
    def parse_store(self, response):
        ...
        item['store_number'] = self.validate(response.xpath('//div[@class="rio-citylist-locName"]/a/@href')).split('ca')[-1].split('.')[0]
